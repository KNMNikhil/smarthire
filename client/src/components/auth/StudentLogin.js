...
const StudentLogin = () => {
  ...
  const [registerData, setRegisterData] = useState({
    ...
    arrears: 0, higherStudies: false, internship: false, batch: '', placedStatus: 'Not Placed', department: ''
  });
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    // Email validation
    if (!registerData.email.endsWith('@rajalakshmi.edu.in')) {
      alert('Email must end with @rajalakshmi.edu.in');
      return;
    }
    ...
    if (!passwordRegex.test(registerData.password)) {
      alert('Password must be at least 6 characters with 1 uppercase letter, 1 lowercase letter, 1 number, and 1 special character');
      return;
    }
    
    // Roll number validation
    if (!/^\d{9}$/.test(registerData.rollNo)) {
      alert('Roll number must be exactly 9 digits');
      return;
    }
          ...
          <div className="bg-black/95 backdrop-blur-md border border-white/20 shadow-2xl rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleRegisterSubmit} className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-medium text-white mb-1">Email *</label><input type="email" name="email" placeholder="220701120@rajalakshmi.edu.in" required value={registerData.email} onChange={handleRegisterChange} className={`w-full px-3 py-2 text-sm bg-white/10 border rounded focus:outline-none focus:ring-1 text-white placeholder-gray-400 ${emailError ? 'border-red-500 focus:ring-red-500' : 'border-white/20 focus:ring-purple-500'}`} /></div>
                <div><label className="block text-xs font-medium text-white mb-1">Password *</label><PasswordInput name="password" value={registerData.password} onChange={handleRegisterChange} placeholder="Min 6 chars, 1 upper, 1 number, 1 symbol" required /></div>
                <div><label className="block text-xs font-medium text-white mb-1">Confirm Password *</label><div className="relative"><input type={isVisible ? 'text' : 'password'} name="confirmPassword" placeholder="Confirm password" required value={registerData.confirmPassword} onChange={handleRegisterChange} className="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-1 focus:ring-purple-500 text-white placeholder-gray-400" /><button type="button" onClick={() => setIsVisible((prev) => !prev)} className="absolute inset-y-0 right-0 outline-none flex items-center justify-center w-9 text-gray-400 hover:text-white">{isVisible ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></div>
                <div><label className="block text-xs font-medium text-white mb-1">Roll Number *</label><input type="text" name="rollNo" placeholder="9 digits (e.g., 220701120)" required value={registerData.rollNo} onChange={handleRegisterChange} className={`w-full px-3 py-2 text-sm bg-white/10 border rounded focus:outline-none focus:ring-1 text-white placeholder-gray-400 ${rollNoError ? 'border-red-500 focus:ring-red-500' : 'border-white/20 focus:ring-purple-500'}`} /></div>
                <div><label className="block text-xs font-medium text-white mb-1">Date of Birth *</label><input type="date" name="dob" required value={registerData.dob} onChange={handleRegisterChange} className="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-1 focus:ring-purple-500 text-white [color-scheme:dark]" /></div>
                ...
                <div><label className="block text-xs font-medium text-white mb-1">Department *</label><select name="department" required value={registerData.department} onChange={handleRegisterChange} className="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 rounded focus:outline-none focus:ring-1 focus:ring-purple-500 text-white"><option value="" style={{backgroundColor: '#000', color: '#fff'}}>Select Department</option><option value="CSE" style={{backgroundColor: '#000', color: '#fff'}}>CSE</option><option value="ECE" style={{backgroundColor: '#000', color: '#fff'}}>ECE</option><option value="AIML" style={{backgroundColor: '#000', color: '#fff'}}>AIML</option><option value="AIDS" style={{backgroundColor: '#000', color: '#fff'}}>AIDS</option><option value="EEE" style={{backgroundColor: '#000', color: '#fff'}}>EEE</option><option value="CSBS" style={{backgroundColor: '#000', color: '#fff'}}>CSBS</option><option value="IT" style={{backgroundColor: '#000', color: '#fff'}}>IT</option></select></div>
              </div>
