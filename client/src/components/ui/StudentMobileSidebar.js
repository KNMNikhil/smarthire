...
const StudentMobileSidebar = ({ user, handleLogout }) => {
  ...
  const sidebarItems = [
    { icon: <MessageSquare className="h-5 w-5" />, label: "Chats", href: "/student/chats" },
    { icon: <HelpCircle className="h-5 w-5" />, label: "Queries", href: "/student/queries" },
  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 right-4 z-40 p-2 bg-purple-600 rounded-full shadow-lg"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden fixed right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-gradient-to-br from-gray-900 via-black to-gray-900 border-l border-purple-500/20 z-50 overflow-y-auto"
          >
            {/* Header */}
            <div className="sticky top-0 bg-black/80 backdrop-blur-xl border-b border-purple-500/20 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-white font-semibold">{user?.name || 'Student'}</h3>
